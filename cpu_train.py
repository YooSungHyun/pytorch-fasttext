...
def main(hparams: TrainingArguments):
    ...
    with open("raw_data/500_fasttext_vocab.json", "r") as st_json:
        tokenizer = json.load(st_json)

    with open("raw_data/500_fasttext_train_cnt.json", "r") as st_json:
        train_counter = json.load(st_json)
    with open("raw_data/500_fasttext_eval_cnt.json", "r") as st_json:
